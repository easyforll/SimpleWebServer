...
void WebServer::dealwithread(int sockfd){
    ...
    if(1 == m_actormodel){
        if(timer){
            adjust_timer(timer);  //?? the correspondance between timer and thread
        }
        // when detect read event ,put the event into request queue
        m_pool->append(users + sockfd,0);

        while(true){
            if(1 == users[sockfd].improv){
                if(1 == users[sockfd].timer_flag){    //?? can't know the meaning
